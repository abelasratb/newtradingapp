...
const MarketSessions = ({ langMode }: { langMode: 'english' | 'amharic' }) => {
      ...
      <div className="flex justify-between items-start mb-8 relative z-10">
        <div>
          <h3 className="text-xl font-black text-white flex items-center gap-2.5 uppercase tracking-tighter">
            <Clock size={20} className="text-trade-primary" /> {langMode === 'amharic' ? 'የገበያ ሰዓታት' : 'Session Hub'}
          </h3>
          <p className="text-[10px] text-slate-500 font-black uppercase tracking-[0.3em] mt-1.5 flex items-center gap-2">
            {langMode === 'amharic' ? 'ሁኔታ' : 'Status'}: {isGlobalOpen ? <span className="text-emerald-500 flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" /> {langMode === 'amharic' ? 'ክፍት' : 'LIVE'}</span> : <span className="text-rose-500">{langMode === 'amharic' ? 'ዝግ' : 'CLOSED'}</span>}
          </p>
        </div>
        ...
        {sessions.map(session => {
          ...
          return (
            <div key={session.name} className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-500 ${isOpen ? 'bg-slate-800/80 border-trade-primary/50 shadow-xl' : 'bg-slate-900/30 border-slate-800/50 opacity-40'}`}>
              ...
              {isOpen && <span className="text-[9px] font-black text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-lg border border-emerald-500/20 tracking-widest">{langMode === 'amharic' ? 'ክፍት' : 'TRADING'}</span>}
            </div>
          );
        })}
      </div>
...
export const Dashboard: React.FC<DashboardProps> = ({ progress, chapters, onStartChapter, langMode = 'english' }) => {
    ...
    <div className="space-y-10 animate-fade-in pb-12">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-trade-card p-7 rounded-[40px] border border-slate-800 shadow-2xl group hover:border-trade-primary/30 transition-all">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4">{langMode === 'amharic' ? 'የሞጁል ሂደት' : 'Module Conquest'}</p>
        ...
        <div className="bg-trade-card p-7 rounded-[40px] border border-slate-800 shadow-2xl group hover:border-yellow-500/30 transition-all">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4">{langMode === 'amharic' ? 'የዕውቀት ምዘና' : 'IQ Percentile'}</p>
          <div className="flex items-end justify-between">
        ...
        <div className="bg-trade-card p-7 rounded-[40px] border border-slate-800 shadow-2xl group hover:border-emerald-500/30 transition-all">
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] mb-4">{langMode === 'amharic' ? 'የሁኔታ ማትሪክስ' : 'Status Matrix'}</p>
          <div className="flex items-end justify-between">
            <h3 className="text-3xl font-black text-emerald-500 leading-none uppercase tracking-tighter">{langMode === 'amharic' ? 'ንቁ ኤሊት' : 'Elite Active'}</h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        <div className="lg:col-span-8 space-y-6">
          <div className="bg-trade-card rounded-[50px] border border-slate-800 p-10 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] relative overflow-hidden">
            <div className="flex justify-between items-center mb-12 relative z-10">
              <div>
                <h3 className="text-2xl font-black text-white uppercase tracking-tight flex items-center gap-3"><Zap className="text-trade-primary" size={28} /> {langMode === 'amharic' ? 'የአካዳሚክ ፍኖተ ካርታ' : 'Academic Roadmap'}</h3>
                <p className="text-[11px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1.5">{langMode === 'amharic' ? 'ወደ ትርፋማነት የሚያደርስዎ መንገድ' : 'Your evolutionary path to profitability'}</p>
              </div>
            </div>
              ...
              {chapters.map((ch, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative p-6 rounded-[32px] border transition-all cursor-pointer flex items-center gap-8 ${isCompleted ? 'bg-emerald-500/5 border-emerald-500/20 hover:bg-emerald-500/10' :
                  ...
                  >
                    <div className={`w-14 h-14 rounded-2xl shrink-0 flex items-center justify-center shadow-2xl transition-all group-hover:rotate-6 group-hover:scale-110 ${isCompleted ? 'bg-emerald-500 text-white shadow-emerald-900/40' :
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-[11px] font-black uppercase tracking-[0.3em] mb-1 ${isCurrent ? 'text-trade-primary' : 'text-slate-500'}`}>{langMode === 'amharic' ? `ደረጃ ${ch.id}` : `Level Phase ${ch.id}`}</p>
                      <h4 className="font-black text-white text-lg uppercase truncate tracking-tighter">{langMode === 'amharic' ? ch.titleAmharic : ch.title}</h4>
                    </div>
                    <div className="shrink-0 flex items-center gap-3">
                      {isLocked ? (
                        <div className="px-3 py-1 bg-slate-800 rounded-lg text-[9px] font-black uppercase text-slate-500 tracking-widest">{langMode === 'amharic' ? 'ተቆልፏል' : 'Locked'}</div>
                      ) : (
        ...
        <div className="lg:col-span-4 space-y-8">
          <MarketSessions langMode={langMode as 'english' | 'amharic'} />
          <div className="bg-gradient-to-br from-indigo-950 to-trade-card border border-indigo-500/30 rounded-[40px] p-10 relative overflow-hidden group shadow-2xl shadow-indigo-900/20 cursor-pointer transition-all hover:scale-[1.02]">
            <div className="relative z-10">
              ...
              <p className="text-[11px] font-black text-indigo-400 uppercase tracking-[0.4em] mb-4">{langMode === 'amharic' ? 'የአፈጻጸም ላብራቶሪ' : 'Performance Lab'}</p>
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter leading-tight mb-8">{langMode === 'amharic' ? 'የዋጋ እንቅስቃሴዎች በልቀው ይማሩ' : 'Reflex Edge: Master Price Patterns'}</h3>
              <button className="bg-white text-slate-900 px-8 py-4 rounded-2xl font-black uppercase text-[11px] tracking-widest shadow-2xl group-hover:bg-indigo-500 group-hover:text-white transition-all active:scale-95 w-full">{langMode === 'amharic' ? 'ጀምር' : 'Initialize Session'}</button>
            </div>
